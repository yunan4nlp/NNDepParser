#ifndef ComputionGraph_H_
#define ComputionGraph_H_
...
public:
	...
	inline void decode(Graph *cg, const Instance &inst, const vector<CAction> *gold_actions = NULL) {
		...
		pGenerator->ready(&inst); // get instance data
		...
		while (true) {
			...
			for (int idx = 0; idx < candidate_action_num; idx++) {
				...
				if (scored_action.state->_is_gold && scored_action.ac == gold_answer) {
					...
					if(cg->train) pGenerator->_next_action_score.outputs[idx].val[0] + pOpts->delta;
				}
			...
			if (cg->train) {
				...
				for (int idx = 0; idx < offset; idx++) {
					...
					if (current_scored_action.is_gold) {
						pGenerator = current_scored_action.state;
						//cout << current_scored_action.ac.str(*pOpts) << endl;
						pGenerator->move(&states[step], current_scored_action.ac);
				...
				pGenerator = highest_score_action.state;
				pGenerator->move(&states[step], highest_score_action.ac); // move the highest score
